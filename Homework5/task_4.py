...
def scrabble(word):
    """
    Функция подсчитывает количество баллов за буквы в слове, каждая буква стоит определенное количество баллов
    :param word: строка, которую принимаем на вход
    :return: возвращаем количество баллов за буквы
    """
    # Задаем соответствие строкам и баллам в словаре
    scores = {'а, в, е, ё, и, н, о, р, с, т': 1,
              'д, к, л, м, п, у': 2,
              'б, г, ь, я': 3,
              'й, ы': 4,
              'ж, з, х, ц, ч': 5,
    ...
    for char in word:
        for key in scores:
            if char in key:
                # Если нашли совпадение, то увеличиваем баллы на значение ключа в котором нашли букву
                points += scores[key]
    return points
